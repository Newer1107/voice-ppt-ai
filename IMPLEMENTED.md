...
| Models | WHISPER_MODEL_SIZE, LLM_MODEL, TTS_SAMPLE_RATE | Model configuration |
...
All models are loaded once at startup in a single container. SGLang (Qwen) runs as a separate companion container.
...
| POST | `/ai/v1/align` | Qwen3-8B via SGLang | Transcript-slide alignment |
| POST | `/ai/v1/generate-narration` | Qwen3-8B via SGLang | Per-slide narration scripts |
...
| Qwen3-8B (SGLang) | `sglang/sglang-openai` container | ~16 GB | N/A |
...
**LLM:**
- Calls SGLang's OpenAI-compatible API (`/v1/chat/completions`) with JSON response format
...
| `sglang` | `sglang/sglang-openai:latest` | Required | 8000 |
...
└────────────────────────┬────────────────────────────────────┘
                         ...
                         ▼
┌─────────────────────────────────────────────────────────────┐
│  Stage 6: Generate Narration (Qwen via SGLang — GPU Server)  │
...
All AI inference is served by a single unified service. SGLang (Qwen) runs as a companion container.
...
| GET | `/ai/v1/dimensions` | BAAI/bge-m3 | Embedding dimensionality |
| POST | `/ai/v1/align` | Qwen3-8B via SGLang | Transcript-slide alignment |
| POST | `/ai/v1/generate-narration` | Qwen3-8B via SGLang | Per-slide narration generation |
| POST | `/ai/v1/tts` | F5-TTS | Speech synthesis (WAV) |
...
| Real Qwen narration generation (SGLang + prompt, unified GPU service) | ✅ |
